...
<<<<<<< Updated upstream
 ...
 const ShowUsers = ({ resp }) => {
  return (
    <ul>
      {resp.map(user => <li key={user.name}>{user.name}</li>)}
    </ul>
  )
}}
...
function handleHomeClick(event, hist) {
    console.log(event)
    event.preventDefault()
    event.stopPropagation()
    hist.push("/");
...
export const Vaccinationlisting = () => {
    ...
    return (
        <>
            ...
            <Rowtext>
                <Textcontent>
                    <TitleHead> O-Zone</TitleHead>
                    {/* <Texthead>Want to buy Oxygen?</Texthead> */}
                </Textcontent>
                <button type="button" class="btn btn-warning c1 c" onClick = {(e) => handleHomeClick(e, history)}>Home</button>
            ...
            <Bottom>
                <Half>
                    ...
                    <form id='form' className='search'>
                        <div id='inmat'>
                            <input id='inp' placeholder='Search Vaccination centers ' ref={textInput} />
                        </div>
